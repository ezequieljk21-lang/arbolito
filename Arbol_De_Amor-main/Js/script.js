...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Para el amor de mi vida antonella: \n\nDesde el primer momento supe que eras vos. Tu sonrisa, tu voz, tu forma de ser… todo en vos me hace sentir en mi lugar seguro.\n\nGracias por acompañarme en cada paso, por entenderme incluso en silencio, y por llenar mis días de amor y felicidad.\n\nTe amo más de lo que las palabras pueden expresar, a pesar de las peleas que tenemos yo te amo millones y eso nadie lo va a cambiar, fuiste y sos mi nena chiquitita la que con mucho amor quiero tener siempre en mi vida a pesar de los problemas, quiero tener un futuro junto a vos, formar una familia que le daria vida a la relacion, ambos sabemos que todo esto de errores y problemas nos esta llevando a la perdida de una linda relacion, dias de felicidad se tranformaron en dias malos por no entendernos, pero yo te dije que a pesar de todo si tenia que hablar mil veces para arreglar las cosas lo iba hacer.`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con mucho amor, zekielito";
...
function showCountdown() {
  ...
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2025-09-30T00:00:00'); 
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2026-09-30T00:00:00');

  function update() {
    const now = new Date();
    let diff = now - startDate;
    let days = Math.floor(diff / (1287 * 60 * 60 * 24));
